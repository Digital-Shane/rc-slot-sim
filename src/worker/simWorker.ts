...
import {
  ...
  roundToCents,
  sampleMultiplier,
} from '../sim/volatilityModel';
import { createRng } from '../sim/rng';

const MODEL_VERSION = 'slot-volatility-v2' as const;
...
async function runSimulation(inputs: SimulationInputs): Promise<SimulationResult> {
  const rtpMean = normalizeRtp(inputs.rtp_percent);
  const distribution = buildCalibratedDistribution(inputs.volatility, rtpMean);
  ...
  for (let runIndex = 1; runIndex <= inputs.runs; runIndex += 1) {
    ...
    while (coinInCents < targetCents) {
      ...
      balance -= bet;
      const payout = samplePayout(bet, distribution, rng);
...
function samplePayout(
  bet: number,
  distribution: ReturnType<typeof buildCalibratedDistribution>,
  rng: () => number
): number {
  const multiplier = sampleMultiplier(distribution, rng);
  return roundToCents(bet * multiplier);
}
